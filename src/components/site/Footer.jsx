...
export function Footer() {
    return (<footer className="border-t border-border/60 bg-surface">
      <div className="mx-auto grid max-w-7xl gap-12 px-6 py-16 lg:grid-cols-[1.2fr_1fr_1fr] lg:px-10">
        ...
        <div>
          ...
          <ul className="mt-4 space-y-3 text-sm">
            {projects.map((p) => (<li key={p.slug}>
                <Link to="/projects/$slug" params={{ slug: p.slug }} className="text-muted-foreground transition-colors hover:text-brand">
                  {p.name}
                </Link>
              </li>))}
          </ul>
        ...
        <div>
          ...
          <address className="mt-4 space-y-3 text-sm not-italic text-muted-foreground">
            ...
            <p>
              ...
              <br />
              <a className="transition-colors hover:text-brand" href="mailto:hello@ellikaventures.com">
