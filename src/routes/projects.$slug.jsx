...
import { ArrowLeft, ArrowRight, Check, MapPin, FileText, Phone, ShieldCheck, Calendar, Sparkles, } from "lucide-react";
...
function ProjectNotFound() {
    return (<div className="mx-auto max-w-3xl px-6 py-44 text-center">
...
function ProjectDetail() {
    ...
    const [modalAction, setModalAction] = useState("brochure");
    const [selectedUnitTitle, setSelectedUnitTitle] = useState(undefined);
    const openModal = (action, unitTitle) => {
    ...
    return (<article className="min-h-screen bg-background pb-20 max-w-full">
      {/* HERO SECTION */}
      <section className="relative flex min-h-[85vh] items-end overflow-hidden">
        <img src={project.image} alt={`${project.name} in ${project.location}`} width={1920} height={1100} className="absolute size-full object-cover"/>
        ...
        <div className="relative mx-auto w-full max-w-7xl px-4 sm:px-6 pb-16 lg:px-10 lg:pb-20">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <Link to="/projects" className="inline-flex items-center gap-2 text-xs uppercase tracking-[0.2em] text-white/80 transition-colors hover:text-brand">
          ...
          <div className="mt-8 flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            <button onClick={() => openModal("brochure")} className="inline-flex items-center justify-center gap-2 rounded-md bg-gradient-brand px-6 py-3.5 text-xs uppercase tracking-[0.2em] text-brand-foreground shadow-brand hover:opacity-95">
              <FileText className="size-4"/>
              <span>Download Brochure</span>
            </button>
            <button onClick={() => openModal("visit")} className="inline-flex items-center justify-center gap-2 rounded-md border border-brand/60 bg-surface/90 px-6 py-3.5 text-xs uppercase tracking-[0.2em] text-foreground backdrop-blur-sm transition-all hover:bg-brand hover:text-brand-foreground">
      ...
      <nav className="sticky top-20 z-30 border-y border-border/80 bg-background/95 backdrop-blur-lg">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 sm:px-6 py-3 lg:px-10">
          <div className="flex items-center gap-6 overflow-x-auto no-scrollbar py-1 pr-8">
            {subNavLinks.map((link) => (<a key={link.label} href={link.href} className="whitespace-nowrap text-xs uppercase tracking-[0.18em] text-muted-foreground transition-colors hover:text-brand">
                {link.label}
              </a>))}
          </div>
          ...
          <div className="hidden items-center gap-3 sm:flex">
            ...
            <button onClick={() => openModal("pricing")} className="rounded bg-brand px-3.5 py-1.5 text-[0.6875rem] uppercase tracking-wider text-brand-foreground">
              Get Cost Sheet
            </button>
          </div>
        </div>
      ...
      <section className="border-b border-border/60 bg-surface">
        <div className="mx-auto grid max-w-7xl grid-cols-2 gap-4 sm:gap-6 px-4 sm:px-6 py-8 sm:py-10 sm:grid-cols-4 lg:px-10">
          {quickFacts.map((f) => (<div key={f.label} className="min-w-0">
        ...
        <div className="min-w-0 max-w-full space-y-14 sm:space-y-20">
          ...
          <section id="overview" className="scroll-mt-36">
            ...
            <div className="mt-6 space-y-4 text-base leading-relaxed text-muted-foreground">
              {project.description.map((p) => (<p key={p.slice(0, 30)}>{p}</p>))}
            </div>
            ...
            <div className="mt-12 rounded-xl border border-border/80 bg-surface p-6 sm:p-8">
              ...
              <ul className="mt-6 grid gap-4 sm:grid-cols-2">
                {project.highlights.map((h) => (<li key={h} className="flex items-start gap-3 text-sm leading-relaxed text-muted-foreground">
          ...
          <section id="plans" className="scroll-mt-36">
            <ProjectFloorPlans masterPlan={project.masterPlan} unitPlans={project.unitPlans} projectName={project.name} onDownloadFloorPlan={(unitTitle) => openModal("floorplan", unitTitle)}/>
          </section>

          {/* 3. AMENITIES WITH IMAGES */}
          <section id="amenities" className="scroll-mt-36">
            <ProjectAmenities visualAmenities={project.visualAmenities} allAmenities={project.amenities} projectName={project.name} onEnquire={() => openModal("brochure")}/>
          </section>
          ...
          <section id="location" className="scroll-mt-36">
            <ProjectMap mapData={project.mapData} projectName={project.name} location={project.location} categories={project.categorizedConnectivity}/>
          </section>
          ...
          <section id="pricing" className="scroll-mt-36">
            <div className="rounded-xl border border-border/80 bg-surface p-4 sm:p-8 min-w-0 max-w-full overflow-hidden">
              <div className="flex flex-wrap items-end justify-between gap-4">
                ...
                <button onClick={() => openModal("pricing")} className="rounded-md bg-gradient-brand px-5 py-2.5 text-xs uppercase tracking-[0.18em] text-brand-foreground shadow-brand">
                  Request Cost Sheet
                </button>
              </div>
              ...
              <div className="mt-2 sm:mt-6 overflow-x-auto rounded-lg border border-border/70">
                <table className="w-full min-w-[28rem] text-left text-sm">
                  ...
                  <tbody className="divide-y divide-border/60">
                    {project.unitMix.map((u) => (<tr key={u.type} className="hover:bg-background/40">
                        ...
                        <td className="px-5 py-4 text-right">
                          <button onClick={() => openModal("floorplan", u.type)} className="text-xs uppercase tracking-wider text-brand hover:underline">
                            View Layout
                          </button>
                        </td>
                      </tr>))}
        ...
        <aside className="lg:sticky lg:top-36 lg:self-start space-y-6">
          <div className="rounded-xl border border-brand/40 bg-surface p-7 shadow-lift">
            ...
            <dl className="mt-6 space-y-3 border-y border-border/60 py-4 text-xs">
              {allFacts.map((f) => (<div key={f.label} className="flex justify-between gap-4">
            ...
            <div className="mt-6 space-y-3">
              <button onClick={() => openModal("brochure")} className="flex w-full items-center justify-center gap-2 rounded-md bg-gradient-brand py-3.5 text-xs font-medium uppercase tracking-[0.22em] text-brand-foreground shadow-brand hover:opacity-95">
                <FileText className="size-4"/>
                <span>Download Brochure</span>
              </button>

              <button onClick={() => openModal("visit")} className="flex w-full items-center justify-center gap-2 rounded-md border border-brand/60 bg-background py-3.5 text-xs font-medium uppercase tracking-[0.22em] text-foreground transition-all hover:bg-brand hover:text-brand-foreground">
                <Calendar className="size-4 text-brand"/>
                <span>Schedule Site Visit</span>
              </button>

              <a href="tel:+919876543210" className="flex w-full items-center justify-center gap-2 rounded-md border border-border py-3 text-xs uppercase tracking-[0.18em] text-muted-foreground transition-colors hover:border-brand hover:text-brand">
      ...
      <section className="border-t border-border/60 bg-surface">
        <div className="mx-auto max-w-7xl px-6 py-20 lg:px-10">
          <div className="flex flex-wrap items-end justify-between gap-4">
            ...
            <Link to="/projects" className="inline-flex items-center gap-2 text-xs uppercase tracking-[0.2em] text-brand hover:underline">
          ...
          <div className="mt-8 grid gap-6 sm:grid-cols-3">
            {others.map((p) => (<Link key={p.slug} to="/projects/$slug" params={{ slug: p.slug }} className="group overflow-hidden rounded-xl border border-border/60 bg-background transition-all hover:border-brand/60 hover:shadow-lift">
                <div className="relative h-48 w-full overflow-hidden">
                  <img src={p.image} alt={p.name} width={800} height={500} loading="lazy" className="size-full object-cover transition-transform duration-500 group-hover:scale-105"/>
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 border-t border-border/80 bg-surface/95 px-4 pt-3 pb-[max(0.875rem,env(safe-area-inset-bottom))] shadow-2xl backdrop-blur-md lg:hidden">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => openModal("brochure")} className="rounded-md border border-brand/60 px-3 py-2 text-[0.6875rem] uppercase tracking-wider text-foreground">
              Brochure
            </button>
            <button onClick={() => openModal("visit")} className="rounded-md bg-gradient-brand px-4 py-2 text-[0.6875rem] uppercase tracking-wider text-brand-foreground shadow-brand">
              Site Visit
            </button>
          </div>
        </div>
      ...
      <EnquiryModal isOpen={modalOpen} onClose={() => setModalOpen(false)} projectName={project.name} defaultAction={modalAction} unitTitle={selectedUnitTitle}/>
    </article>);
}
