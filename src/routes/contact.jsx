...
const description = "Request a brochure or book an accompanied site visit for Ellikaventures projects in Bengaluru. We reply within one working day.";
export const Route = createFileRoute("/contact")({
    validateSearch: (search) => typeof search["project"] === "string" ? { project: search["project"] } : {},
    head: () => ({
        meta: [
            { title },
...
const inputClass = "mt-2 w-full rounded-md border border-input bg-background px-4 py-3 text-sm text-foreground outline-none transition-colors placeholder:text-muted-foreground/70 focus:border-brand";
function Contact() {
    const { project } = Route.useSearch();
    const [sent, setSent] = useState(false);
    return (<div className="mx-auto max-w-7xl px-6 pb-28 pt-36 lg:px-10 lg:pt-44">
      <div className="grid gap-16 lg:grid-cols-[1fr_1.1fr]">
        <div>
          ...
          <dl className="mt-12 space-y-6 text-sm">
            ...
            <div>
              ...
              <dd className="mt-2">
                <a href="mailto:hello@ellikaventures.com" className="text-muted-foreground hover:text-brand">
        ...
        <div className="rounded-lg border border-border/70 bg-surface p-8 lg:p-10">
          {sent ? (<div className="flex h-full flex-col items-center justify-center py-16 text-center">
              ...
              <button onClick={() => setSent(false)} className="mt-8 text-xs uppercase tracking-[0.2em] text-brand">
                Send another
              </button>
            </div>) : (<form className="space-y-6" onSubmit={(e) => {
                e.preventDefault();
                setSent(true);
            }}>
              ...
              <label className="block text-xs uppercase tracking-[0.2em] text-muted-foreground">
                Email
                <input required type="email" name="email" className={inputClass} placeholder="you@email.com"/>
              </label>
              <div className="grid gap-6 sm:grid-cols-2">
                <label className="block text-xs uppercase tracking-[0.2em] text-muted-foreground">
                  Project of interest
                  <select name="project" className={inputClass} defaultValue={project ?? ""}>
                    ...
                    {projects.map((p) => (<option key={p.slug} value={p.slug}>
                        {p.name} — {p.location}
                      </option>))}
                  </select>
                </label>
              ...
              <label className="block text-xs uppercase tracking-[0.2em] text-muted-foreground">
                Message
                <textarea name="message" rows={4} className={inputClass} placeholder="Preferred locality, commute, timeline…"/>
              </label>
              <button type="submit" className="w-full rounded-md bg-gradient-brand px-6 py-4 text-xs uppercase tracking-[0.22em] text-brand-foreground shadow-brand">
                Send enquiry
              </button>
            </form>)}
        </div>
