...
import { ArrowRight, Search, ShieldCheck, FileText, Phone, CheckCircle2, Calendar, } from "lucide-react";
...
const description = "Authorised partner for premium Bengaluru residences. Discover curated 2, 3 and 4 BHK apartments, luxury villas, and row houses with verified RERA documentation, master plans, and site visits.";
...
function SearchBar() {
    ...
    const selectClass = "w-full appearance-none bg-white px-4 py-3.5 sm:py-4 text-xs font-medium uppercase tracking-wider text-neutral-800 outline-none transition cursor-pointer";
    return (<form onSubmit={(e) => {
      ...
      <button type="submit" className="flex items-center justify-center gap-2 bg-gradient-brand px-6 py-3.5 sm:py-4 text-xs font-semibold uppercase tracking-[0.22em] text-brand-foreground transition hover:opacity-95">
...
function Index() {
    ...
    const filteredProjects = selectedFilter === "All" ? projects : projects.filter((p) => p.propertyType === selectedFilter);
    const openEnquiry = (projectName) => {
    ...
    return (<div className="bg-background">
      ...
      <section className="relative flex min-h-[92vh] items-center overflow-hidden">
        <img src={hero} alt="Lakeside luxury residential towers in Bengaluru" width={1920} height={1200} className="absolute size-full object-cover"/>
      ...
      <section className="mx-auto max-w-7xl px-6 py-24 lg:px-10 lg:py-28">
        <div className="flex flex-wrap items-end justify-between gap-6">
          ...
          <div className="flex flex-wrap gap-2">
            {["All", "Apartments", "Villas", "Row Houses"].map((tab) => (<button key={tab} onClick={() => setSelectedFilter(tab)} className={`rounded-md px-4 py-2 text-xs uppercase tracking-[0.16em] transition-all ${selectedFilter === tab
        ...
        <div className="mt-12 grid gap-8 md:grid-cols-2">
          {filteredProjects.map((p, idx) => (<ProjectCard key={p.slug} project={p} index={idx}/>))}
        </div>
      </section>
      ...
      <section className="border-t border-border/80 bg-surface py-24">
        <div className="mx-auto max-w-7xl px-6 lg:px-10">
          <div className="grid gap-12 lg:grid-cols-2 lg:items-center">
            ...
            },
            {
        ...
        ].map((feat) => (<div key={feat.t} className="rounded-lg border border-border/60 bg-background p-4">
              ...
              <div className="mt-8">
                <button onClick={() => openEnquiry("Curated Portfolio")} className="inline-flex items-center gap-2 rounded-md bg-gradient-brand px-6 py-3.5 text-xs uppercase tracking-[0.2em] text-brand-foreground shadow-brand hover:opacity-95">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-4">
                <img src={clubhouse} alt="Modern Clubhouse" width={600} height={700} className="h-64 w-full rounded-xl object-cover shadow-lift"/>
                <img src="https://images.unsplash.com/photo-1512917774080-9991f1c4c750?auto=format&fit=crop&w=600&q=80" alt="Infinity Pool" width={600} height={500} className="h-44 w-full rounded-xl object-cover shadow-lift"/>
              </div>
              <div className="space-y-4 pt-8">
                <img src={interior} alt="Luxury Ivory Interior" width={600} height={500} className="h-44 w-full rounded-xl object-cover shadow-lift"/>
                <img src="https://images.unsplash.com/photo-1600585154340-be6161a56a0c?auto=format&fit=crop&w=600&q=80" alt="Private Balcony & Courtyard" width={600} height={700} className="h-64 w-full rounded-xl object-cover shadow-lift"/>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-24 lg:px-10">
        ...
        ].map((loc) => (<div key={loc.hub} className="rounded-xl border border-border/70 bg-surface p-6 shadow-sm transition-all hover:border-brand/60 hover:shadow-lift">
      ...
      <section className="border-t border-border/80 bg-surface py-24">
        ...
        ].map((st) => (<div key={st.step} className="rounded-xl border border-border/70 bg-background p-7 shadow-sm transition hover:border-brand/60">
      ...
      <section className="mx-auto max-w-5xl px-6 py-24 text-center lg:px-10">
        <div className="rounded-2xl border border-brand/50 bg-gradient-to-b from-surface to-background p-10 shadow-2xl sm:p-16">
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-4">
            <button onClick={() => openEnquiry("Private Site Visit")} className="inline-flex items-center gap-2 rounded-md bg-gradient-brand px-8 py-4 text-xs font-semibold uppercase tracking-[0.22em] text-brand-foreground shadow-brand hover:opacity-95">
              <Calendar className="size-4"/>
              <span>Book a Private Visit</span>
            </button>
            <a href="tel:+919876543210" className="inline-flex items-center gap-2 rounded-md border border-border bg-surface px-8 py-4 text-xs uppercase tracking-[0.2em] text-foreground transition hover:border-brand hover:text-brand">
      ...
      {/* MODAL */}
      <EnquiryModal isOpen={modalOpen} onClose={() => setModalOpen(false)} projectName={activeProjectForModal} defaultAction="visit"/>
    </div>);
}
